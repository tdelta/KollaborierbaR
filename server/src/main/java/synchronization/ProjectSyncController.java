...
/**
 * Keeps track of clients, which want to be updated on changes to projects saved on the server (see
 ...
 *
 * @todo since {@link server.ProofSyncController} there exists a new base class {@link
 *     server.SyncController} which helps handling subscriptions. Therefore, this class should be
...
public class ProjectSyncController {
   ...
   *
   * <p>Mapping: Project Name -> Editing Users
   */
   ...
   *
   * <p>Mapping: SubscriptionId@User -> Project name
   */
  private ConcurrentHashMap<String, String> subscriptions = new ConcurrentHashMap<>();

  /**
   * Helper method, stores a new association, of a client and their subscription id to a project.
   *
  ...
  /**
   * Helper method, removes a client from receiving changes for a project by removing its
   * subscriptions.
   *
   * @param simpSubscriptionId id STOMP generates for a subscription per user.
   * @param user unique identifier for a client connected via websocket to the server.
  ...
  /**
   * Helper method, retrieves the name of a project a user is subscribed to, associated with the
   * given STOMP subscription id.
   *
   ...
   * @param projectName name of the project for which the client wants to receive notifications
   *     about changes
   */
   ...
   * of being removed from a single project, the client will be removed from all projects they
   * subscribed to.
   *
  ...
  /**
   * Called, whenever *something* changed about the structure of a project, Currently this includes
   * only the creation of new files.
   *
   * <p>It will inform all clients subscribed to the project about the change by forwarding the
   * received event.
   *
   * @param event indicates a change to the structure of a project. Contains information, which
   *     project changed.
   */
  @EventListener
   ...
   *
   * <p>(Event is usually issued by {@link synchronization.SynchronizationController})
   *
   * <p>It will inform all clients subscribed to the project about the new user by forwarding the
   * received event.
   *
  ...
  public void handleOpenedFile(final FileOpenedEvent event) {
    ...
    for (final ConcurrentHashMap.Entry<String, List<Principal>> projectEntry :
        sessions.entrySet()) {
  ...
  /** @return a list of clients, which are currently working on the specified project */
