...
export default class AnchoredMarker{  
  ...
  constructor(range: ace_types.Ace.Range, message: string, type: string,editSession: ace_types.Ace.EditSession){
   ...
   */
  public setRange(range: ace_types.Ace.Range, editSession: ace_types.Ace.EditSession){
    ...
    this.start = editSession.getDocument().createAnchor(range.start.row,range.start.column);
  ...
  for(let i = 0;i < markers.length;i++){
    ...
    switch(existingRange.compareRange(range)){
      ...
      default:    
        ...
        if(existingRange.isStart(range.end.row,range.end.column) && type === markers[i].type){
          range.end = existingRange.end;
          markers.splice(i,1);
          i--;
        }
