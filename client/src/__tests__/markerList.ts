...
test('No range from the inputs gets left empty in the result and no ranges overlap', () => {
  fc.assert(
    ...
    fc.property(
      ...
      (rangesInit: number[][]) => {
        const ranges: ace_types.Ace.Range[] = createRanges(rangesInit);
        const lines: string[] = createText(rangesInit);
...
const createRanges = (rangesInit: number[][]) => {
  const ranges: ace_types.Ace.Range = [];
...
const createText = (rangesInit: number[][]) => {
  const lines: string[] = [];
  // Create a text that contains all ranges
  for (const range of rangesInit) {
    for (let i: number = 0; i <= range[0]; i = i + 1) {
      if (!lines[i]) lines[i] = '';
    }
    for (let i: number = 0; i <= range[2]; i = i + 1) {
      if (!lines[i]) lines[i] = '';
    }
