...
test('No range from the inputs gets left empty in the result and no ranges overlap',() => {
  fc.assert(
    ...
    fc.property(fc.array(fc.array(fc.integer(0,100),4,4),1,10),
    (rangesInit: number[][]) => {
      ...
      for(const range of rangesInit){
        for(let y: number = range[0];y < range[2];y++){
          ...
          for(; x < lines[y].length; x++){
            expect(numContainingMarkers({row: y, column: x},anchoredMarkers,editSession)).toBeLessThanOrEqual(2);
          }
        }
        let x: number = 0;
        for(; x < range[3]; x++){ 
          expect(numContainingMarkers({row: range[2], column: x},anchoredMarkers,editSession)).toBeLessThanOrEqual(2);
        }
      }
    })
  );
...
const numContainingMarkers = function(
  ...
  for(const marker of anchoredMarkers){
    if(marker.getRange(editSession).contains(position))
      containedIn ++;
  }
