...
ace.define(
  ...
  function(acequire, exports) {
    ...
    const TextHighlightRules = acequire('ace/mode/text_highlight_rules')
      .TextHighlightRules;
    const JavaExHighlightRules = acequire(
      'ace/mode/custom_java_highlight_rules'
    ).JavaHighlightRules;
    const lang = acequire('../lib/lang');

    var SequentHighlightRules = function() {
      let keywords = lang.arrayToMap(['measuredByEmpty', 'wellFormed']);

      let literals = lang.arrayToMap(['TRUE', 'FALSE', 'null', 'heap']);
      ...
      this.embedRules(JavaExHighlightRules, 'java-ex-', [javaEndRule]);
    };
