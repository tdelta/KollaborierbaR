const getSyntax = lang => {
  return {
    ...
    keywords: {
      behavior_keyword: lang.arrayToMap(
      ...
      when_keyword: lang.arrayToMap('when|when_redundantly'.split('|')),
    ...
    predicates: {
      predicate_keyword: lang.arrayToMap(
        (
          ...
          '\\only_called|\\only_captured|\\pre|\\product|\\reach|\\real|\\result|\\same|\\safe_math|\\space|\\such_that|\\sum|\\typeof|\\type|\\warn_op|\\warn|\\working_space'
        ).split('|')
      ),
    },
    types: {
